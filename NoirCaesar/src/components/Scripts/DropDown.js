...
export default class ModalDropdown extends Component {
  ...
  _renderButton() {
    const { disabled, accessible, children, textStyle, isStaticButton, defaultValue } = this.props;
          ...
          (
            <View style={styles.button}>
              <Text allowFontScaling={false} style={[styles.buttonText, textStyle]}
                numberOfLines={1}
              >
                {isStaticButton ? defaultValue : buttonText}
              </Text>
            </View>
          )
        }
...
const styles = StyleSheet.create({
  ...
  dropdown: {
    position: 'absolute',
    top: -100,
    height: (33 + StyleSheet.hairlineWidth) * 6,
    borderRadius: 6,
