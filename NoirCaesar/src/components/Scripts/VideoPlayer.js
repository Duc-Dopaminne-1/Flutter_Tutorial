...
import BACK from '@src/res/img/back.png';
import LOADER_ICON from '@src/res/img/loader-icon.png';
import ERROR_ICON from '@src/res/img/error-icon.png';
import TV_ICON from '@src/res/img/top-vignette.png';
import VOLUME from '@src/res/img/volume.png';
import BV_ICON from '@src/res/img/bottom-vignette.png';
import { WIDTH, HEIGHT, colors } from '@src/constants/vars'
import { MediaDetails } from '../Music/MusicPlayerDetails';
...
export default class VideoPlayer extends Component {
  ...
  constructor(props) {
    ...
    this.events = {
      ...
      onPressMore: this.props.onPressMore || this._onPressMore.bind(this),
    ...
    this.methods = {
      ...
      onPressMore: this._onPressMore.bind(this),
  ...
  _onProgress(data = {}) {
    ...
    if (!state.scrubbing) {
      ...
      if (!state.seeking) {
        let position = this.calculateSeekerPosition();
   ...
   */onEnd
  _onEnd = () => {
    if (Platform.OS === 'android' && this.state.currentTime === this.state.duration && this.state.paused) {
      return
    }

    this.setState({ originallyPaused: true, seeking: false, paused: true, currentTime: this.state.duration, isEnd: true, seekerFillWidth: this.player.seekerWidth, seekerPosition: this.player.seekerWidth })
  }
  ...
  _onPrev() {
    let state = this.state;
    this.seekTo(state.currentTime - 5 < 0 ? 0 : state.currentTime - 5)
  }

  _onNext() {
    let state = this.state;
    this.seekTo(state.currentTime + 5 > state.duration ? state.duration : state.currentTime + 5)
  }
  ...
  _togglePlayPause() {
    ...
    if (this.state.isEnd || this.state.currentTime === this.state.duration) {
      this.seekTo(0);
    ...
    this.player.seekPanResponder = PanResponder.create({
      ...
      onPanResponderRelease: (evt, gestureState) => {
        ...
        if (time >= state.duration && !state.loading) {
          ...
          state.seeking = false;
          state.seekerOffset = state.seekerPosition;
          state.isEnd = true
          state.paused = true
          state.originallyPaused = true
          this.events.onEnd();
  ...
  renderTopControls() {
    ...
    const volumeControl = this.props.disableVolume
      ? this.renderNullControl()
      : this.renderVolume();
    const moreControl = this.props.disableMore
      ? this.renderNullControl()
      : this.renderMore();
      ...
      <Animated.View
        ...
        <ImageBackground
          ...
          <SafeAreaView style={styles.controls.topControlGroup}>
            <View style={styles.controls.darkLayout} />
            {backControl}
   ...
   */
  renderMore() {
    let source = this.props.moreIcon
    return this.renderControl(
      <Image source={source} />,
      this.events.onPressMore,
  ...
  renderSeekbar() {
    return (
      <View style={styles.seekbar.mainContainer}>
        <Text style={styles.timer.textLeft}>{this.formatTime(this.state.currentTime)}</Text>
        <View
          ...
          <View
            ...
            onLayout={event => {
              ...
              if (this.state.isEnd) {
                this.setState({ seekerFillWidth: this.player.seekerWidth, seekerPosition: this.player.seekerWidth })
              }
            }
            }
            pointerEvents={'none'}>
        ...
        <Text style={styles.timer.textRight}>{this.formatTime(this.state.duration)}</Text>
      </View>
    );
  }
  ...
  renderPrev() {
    let source = this.props.prevIcon
    return this.renderControl(
      <Image source={source} />,
      this.methods.onPrev,
      styles.controls.prevNext,
    );
  }

  renderNext() {
    let source = this.props.nextIcon
    return this.renderControl(
      <Image source={source} />,
      this.methods.onNext,
      styles.controls.prevNext,
