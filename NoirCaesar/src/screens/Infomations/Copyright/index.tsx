...
const Copyright = () => {
  ...
  const content = useSelector<RootState, string>((state: RootState) => state.pageContent.copyright.content);

  useEffect(() => {
    disatch(
      getPageContent({
        page_type: PageTypeListEnum.Copyright,
      }),
    );
  }, []);
  ...
  return (
    <Container>
      <CustomHeader title={translate('information.copyright')} leftImage={BACK} leftAction={onBack} />
      <ScrollView>
