...
export default class ModalDropdown extends Component {
      ...
      <TouchableOpacity ref={button => (this._button = button)} disabled={disabled} accessible={accessible} onPress={this._onButtonPress}>
        {children || (
          <View style={styles.button}>
            <Text allowFontScaling={false} style={[styles.buttonText, textStyle]} numberOfLines={1}>
              {buttonText}
            </Text>
          </View>
        )}
  ...
  _renderModal() {
    // @ts-ignore
    const { animated, accessible, dropdownStyle, options } = this.props;
    // @ts-ignore
        ...
        >
          <TouchableWithoutFeedback accessible={accessible} disabled={!showDropdown} onPress={this._onModalPress}>
            <View style={styles.modal}>
              <View style={[styles.dropdown, dropdownStyle, frameStyle, { height: (40 + StyleSheet.hairlineWidth) * options.length }]}>
  ...
  _onRowPress(rowData: any, sectionID: any, rowID: any, highlightRow: any) {
    // @ts-ignore
  ...
  _renderSeparator = (sectionID: any, rowID: any, adjacentRowHighlighted: any) => {
