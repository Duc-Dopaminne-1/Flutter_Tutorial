...
const DocumentDetail = (props: Props) => {
  ...
  const onLoadFile = (page: number, onLoadSuccess: () => void, onLoadFailure: () => void) => {
  }
  ...
  const onPressFiles = ({ item, index }: { item: any; index: number }) => {
    ...
    return (
      <View style={styles.containers}>
        ...
        <View style={styles.viewAuthor}>
          <CustomText text={`${translate('document.create_by')} `} style={styles.createBy} styleContainer={styles.titleContainers} />
          <CustomText text={`${item.creator.first_name} ${item.creator.last_name}`} style={styles.author} styleContainer={styles.titleContainers} />
          <CustomText text={`, ${moment(item.modified).format(Config.Manager.formatDate)}`} style={styles.createBy} styleContainer={styles.titleContainers} />
        </View>
        <CustomText numberOfLines={0} text={item.description} style={styles.description} styleContainer={styles.titleContainers} />
        {renderImages()}
        {renderFiles()}
      </View>
    )
