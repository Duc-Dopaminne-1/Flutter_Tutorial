...
import { clone, concat, find, filter } from 'lodash';
...
import { updateFacility } from '@src/modules/FrontDesk/actions';
...
const EditFacility = () => {
  ...
  const { item } = useRoute().params as ItemProps;
  const { flatList } = useRoute().params as any;
  let editFacilityObservable: any = null;
  ...
  const property = buildingList && buildingList.find(itemProperty => itemProperty.id === item.property?.id)
  ...
  const [images, setImages] = useState<ImageUploadModel[]>(item.image_urls && item.image_urls.map(item => ({
    file: {
      uri: item,
    },
    isUploaded: true
  ...
  const [selectedListCountry, setSelectedListCountry] = useState<string[]>([infoFacility.country]);
  const [selectedListBuilding, setSelectedListBuilding] = useState<string[]>([infoFacility.building]);
  ...
  useEffect(() => {
    const dropDownList = buildingList.map(obj => {
      return { _key: `${obj.id}`, _value: obj.name }
    })
    setBuildings(dropDownList);
  }, [buildingList])
  ...
  useEffect(() => {
    ...
    isShowKeyboard = false;
    return () => {
      if (editFacilityObservable) {
        editFacilityObservable.unsubscribe()
      }
  ...
  const renderHeaderTitle = () => (
    <CustomSectionHeader
      ...
      title={translate('edit_facility.title_section_header')}
  ...
  const onSaveFacility = (values: any) => {
    ...
    editFacilityObservable = of(source$).pipe(
      switchMap(() => {
  ...
  const renderFields = () => {
    return (
      <Formik initialValues={infoFacility} onSubmit={onSaveFacility} validationSchema={validationSchema}>
            ...
            <View style={[styles.containerInput, !isAndroid() ? { paddingBottom: paddingBottom } : {}]}>
              ...
              <View style={styles.buttonContainer}>
                <CustomButton
                  ...
                  text={translate('edit_facility.save_facility')}
                  style={styles.button}
                />
              </View>
            </View>
  ...
  return (
    <View style={{ flex: 1 }}>
      <KeyboardAvoidingView style={getStyles('flex-1')} keyboardVerticalOffset={getKeyboardAdvoidStyle()} behavior={'padding'}>
        <Container
          spaceBottom={true}
          title={translate('edit_facility.title_navigation')}
          isShowHeader={true}
...
export default React.memo(EditFacility);
