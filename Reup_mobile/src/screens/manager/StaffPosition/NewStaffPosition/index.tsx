...
import { createPosition } from '@src/modules/Company/actions';
import { RootState } from '@src/types/types';
import { ICompany } from '@reup/reup-api-sdk/libs/api/company/models';
import { CreateCompanyPositionParams } from '@reup/reup-api-sdk/libs/api/company/position/indes';
import NavigationActionsService from '@src/navigation/navigation';
import { IUserProfile } from '@reup/reup-api-sdk/libs/api/user/models';
import { useRoute } from '@react-navigation/native';

const NewStaffPosition = () => {
  ...
  const onCreatePosition = (value: any) => {
    Keyboard.dismiss();
    const selectedCompany = companyList.find(item => item.id === me.default_company.id);
    const currentCompany = selectedCompany ? selectedCompany.id : '';
    const params: CreateCompanyPositionParams = {
  ...
  const renderInputFields = () => {
    return (
      <Formik initialValues={{ positionName: "" }} onSubmit={onCreatePosition} validationSchema={validationSchema}>
        {formikProps => (
          <View style={[styles.containerMain]}>
            <View style={[styles.addStaffPositionContainer]}>
              <CustomSectionHeader
                title={translate('staff.staff_position_section_header')}
                icon={ADD_PLUS}
            ...
            <View style={styles.bottomButtonContainer}>
              <CustomButton
                ...
                text={translate('staff.staff_position_submit')}
    ...
    <View style={[{ flex: 1 }]}>
      <KeyboardAvoidingView style={getStyles('flex-1')} keyboardVerticalOffset={getKeyboardAdvoidStyle()} behavior={'padding'}>
        <Container
          ...
          title={translate('staff.staff_position_add_title')}
        >
          {renderInputFields()}
        </Container>
