...
import LAUNCH_SCREEN_TENANT from '@res/img/splash.png';
...
import { ONBOARDING, MAIN_SCREEN, CONFIRMATION, PERSONAL_INFO, MAINTENANCE_REQUEST, ONBOARDING_TENANT, MAIN_SCREEN_TENANT, CONFIRMATION_TENANT, PERSONAL_INFO_TENANT } from '@src/constants/screenKeys';
...
class App extends React.Component<Props, State> {
  ...
  render() {
    return <Container image={LAUNCH_SCREEN_TENANT} />;
  }
  ...
  toNav = (type: string) => {
    const { userData, isLogging } = this.props;
    ...
    switch (type) {
      ...
      case 'main':
        if (userData && userData.is_updated_profile && isLogging) {
          if (userData && userData.default_property) {
            NavigationActionsService.setRoot(MAIN_SCREEN_TENANT);
          } else {
            NavigationActionsService.setRoot(CONFIRMATION_TENANT);
          }
        } else {
          NavigationActionsService.setRoot(PERSONAL_INFO_TENANT);
        }
        break;
    }
  };
