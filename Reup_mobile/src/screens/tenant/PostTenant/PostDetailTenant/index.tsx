...
import { upperCaseFirstChar, formatCurrency, isTenantApp, isManagerApp, getApartmentName } from '@src/utils';
...
const PostDetailTenant = (props: PostDetailProps) => {
  const route = useRoute();
  const dispatch = useDispatch();
  const { forLeaseSaleFlatList, forLeaseFlatList, forSaleFlatList, data }: { forLeaseSaleFlatList: any, forLeaseFlatList: any, forSaleFlatList: any, data: PostItemModal, isShowBottomButton: boolean } = route.params;
  ...
  const renderBottomButton = () => (
    <View style={styles.bottomButtonView}>
      ...
      <CustomButton onPress={onDeny} text={translate("post.detail_deny_button")} textStyle={styles.denyText} style={[styles.widthButton, styles.denyButton]} />
    </View>
  );
  ...
  const renderPrice = () => {
    return (
      <View style={styles.priceContainer}>
        ...
        {isManagerApp() ? (<CustomText
          numberOfLines={1}
          text={upperCaseFirstChar(data.status ? data.status : '')}
          style={[styles.status,
          data.status === BulletinPostStatus.Pending ? { color: '#1B72BF' } : {},
          data.status === BulletinPostStatus.Approved ? { color: '#1FA207' } : {},
          data.status === BulletinPostStatus.Denied ? { color: '#707070' } : {}]
          }
        />) : null}
      </View>
    );
  ...
  const onCallSeller = () => {
    let phoneNumber = data.creator && data.creator.phone1 ? (data.creator.phone1_code ? data.creator.phone1_code : "") + data.creator.phone1 : "";
  ...
  return (
    <Container isShowHeader={true} title={translate("post.detail_title2")} spaceBottom={true} isDisplayMenuButton={false} isDisplayNotification={false}>
      ...
      {isManagerApp() ? (data.status && data.status === BulletinPostStatus.Pending ? renderBottomButton() : null) : null}
    </Container>
  );

};

export default PostDetailTenant;
