...
function CreateAuctionEntryPrice({ setFieldValue, errors, setFieldError }: FormikValues): ReactElement {
  const [entryPrice, setEndtryPrice] = useState('');

  const onChangeTextEndPrice = (text: string) => {
    if (text === '0.00') {
      setEndtryPrice('');
      setFieldValue('entryPrice', '');
      return;
    }
    setEndtryPrice(text);
    setFieldValue('entryPrice', text);
  };

  const onBlur = () => {
    setFieldError('entryPrice', '');
  };

  return (
    <View style={styles.container}>
      <CreateAuctionTitle title={language('ticketPriceLower')} isRequire subTitle={language('entryPriceLeast')} />
      <TextInputComponent
...
export default memo(CreateAuctionEntryPrice);
