...
import { getCompany, createCompany, updateCompany } from '@/redux/user/actions';
import { useNavigation } from '@react-navigation/native';
import SearchBar from './components/SearchBar';
import { SafeArea } from '@/components/SafeArea';
import { language } from '@/i18n';
import CompanySearchItem from '@/screens/Profile/ProfileEditCompanySearch/components/CompanySearchItem';
...
export function ProfileEditCompanySearch(): ReactElement {
  ...
  useEffect(() => {
    ...
    fetchCompanies().then(_r => undefined);

    return () => {
      currentPageTmp.current = 1;
      perPageTmp = 20;
      keyWorkTmp = '';
    };
  }, []);

  const getCompaniesApi = async (): Promise<any[]> => {
        ...
        getCompany({
          ...
          callback: {
            onSuccess: result => {
              ...
              const { items } = result;
              currentPageTmp.current = currentPage;
              perPageTmp = perPage;
  ...
  const cellOnPressed = async item => {
    if (typeof item === 'string') {
      const result = await dispatch(createCompany({ name: item.trim() }));
      if (result) navigation.goBack();
    } else {
      const result = await dispatch(updateCompany({ companyId: item.id }));
      if (result) navigation.goBack();
    }
  };

  const renderItem = ({ item }) => {
    return <CompanySearchItem item={item} itemOnPressed={cellOnPressed} />;
  };
  ...
  const handleLoadMore = async () => {
    if (!isLoading && !onEndReachedCalledDuringMomentum) {
      ...
      setIsLoading(true);
      currentPageTmp.current = parseInt(String(currentPageTmp.current)) + 1;
      const companies = await getCompaniesApi();
      setIsLoading(false);
      setListData([...listData, ...companies]);
    }
  };
  ...
  const renderContentView = () => {
    if (listData && listData.length > 0) {
      ...
      return <CompanySearchItem item={keyWorkTmp} itemOnPressed={cellOnPressed} />;
    }
  };
  return (
