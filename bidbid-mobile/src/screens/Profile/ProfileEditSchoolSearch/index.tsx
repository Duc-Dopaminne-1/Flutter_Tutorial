...
import { updateSchool, getSchools, createSchool } from '@/redux/user/actions';
import { useNavigation } from '@react-navigation/native';
import SearchBar from './components/SearchBar';
import SchoolSearchItem from './components/SchoolSearchItem';
...
export function ProfileEditSchoolSearchScreen(): ReactElement {
  ...
  useEffect(() => {
    ...
    fetchSchools().then(_r => undefined);
  ...
  const getSchoolsApi = async (): Promise<any[]> => {
  ...
  const onChangeSuggestResult = async (_result: any[], keyWork: string) => {
    ...
    perPageTmp = 20;
    setIsLoading(true);
    const schools = await getSchoolsApi();
    setIsLoading(false);
    setListData([...schools]);
  };
  ...
  const cellOnPressed = async item => {
    if (typeof item === 'string') {
      const result = await dispatch(createSchool({ name: item.trim() }));
      if (result) navigation.goBack();
    } else {
      const result = await dispatch(updateSchool({ schoolId: item.id }));
      if (result) navigation.goBack();
    }
  };

  const renderItem = ({ item }) => {
    return <SchoolSearchItem item={item} itemOnPressed={cellOnPressed} />;
  };
  ...
  const handleLoadMore = async () => {
    if (!isLoading && !onEndReachedCalledDuringMomentum) {
      ...
      setIsLoading(true);
      currentPageTmp = currentPageTmp + 1;
      const schools = await getSchoolsApi();
      setIsLoading(false);
      setListData([...listData, ...schools]);
    }
  };
  ...
  const renderContentView = () => {
    if (listData && listData.length > 0) {
      ...
      return <SchoolSearchItem item={keyWorkTmp} itemOnPressed={cellOnPressed} />;
    }
  };
  return (
