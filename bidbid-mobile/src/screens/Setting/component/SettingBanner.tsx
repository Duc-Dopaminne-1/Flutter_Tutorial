...
export function SettingBanner(): ReactElement {
  ...
  useEffect(() => {
    const shareContactSubscribe = shareContact.subscribe(data => {
      setTitle(data.title);
    ...
    return () => {
      shareContactSubscribe.unsubscribe();
    };
  }, []);
  ...
  return (
    ...
    >
      <TickSVG />
      <Text style={styles.textNote}>{title}</Text>
    </Animated.View>
  );
}
