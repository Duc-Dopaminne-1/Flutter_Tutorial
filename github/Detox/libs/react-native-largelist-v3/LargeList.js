...
export class LargeList extends React.PureComponent<LargeListPropType> {
  ...
  /******  Hack: marginTop in contentContainer create a bug makes user cant scroll to the end
   * => add more height to make up for the margin height */
  getContentAdditionalHeight() {
    const {contentStyle} = this.props;
    if (Array.isArray(contentStyle) && contentStyle.length > 0) {
      let totalAdditionalHeight = 0;
      contentStyle.forEach(e => {
        const {marginTop = 0, marginBottom = 0} = e ?? {};
        totalAdditionalHeight += marginTop + marginBottom;
      });
      return totalAdditionalHeight;
    }
    const {marginTop = 0, marginBottom = 0} = this.props.contentStyle ?? {};
  ...
  render() {
    ...
    const contentStyle =
      sumHeight > 0
        ? {
            height:
              sumHeight + this.getContentAdditionalHeight() > wrapperHeight
                ? sumHeight + this.getContentAdditionalHeight()
    ...
    return (
      <SpringScrollView
        ...
        contentStyle={StyleSheet.flatten([contentStyle, this.props.contentStyle])}
  ...
  _onScroll = e => {
    ...
    this._lastTick = now;
    this._shouldUpdateContent &&
      this._groupRefs.forEach(group => idx(() => group.current.contentConversion(offsetY)));
  };
