...
const ClusteredMapView = ({
  ...
  const renderSpiderMarkers = ()=>{
    if(childSpiderMarkers && spiderMarkers.length){
      return childSpiderMarkers(spiderMarkers)
    }
    return <>
      {spiderMarkers.map(marker => {
        //Canh change for custom maker in spider markers by return propsChildren[marker.index] <<<
        return propsChildren[marker.index] ? (
          React.cloneElement(propsChildren[marker.index], {coordinate: {...marker}})
        ) : (
          <Marker
            key={marker.latitude}
            coordinate={marker}
        ...
        //Canh change for custom maker in spider markers >>>
      })}
