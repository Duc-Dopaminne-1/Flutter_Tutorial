...
const ExtraServiceList = props => {
  const totalCount = useSelector(state => state.extraService?.totalCount);
  const list = useSelector(state => state.extraService?.list);
  const loading = useSelector(state => state.extraService?.loading);
  const [isRefreshing, setRefreshing] = React.useState(false);
  const dispatch = useDispatch();
  const navigation = useNavigation();

  useEffect(() => {
    dispatch(getExtraServiceListHandle({ skipCount: 0 }));
    return () => {};
  }, [dispatch]);
  ...
  const loadMore = () => {
    if (!loading && totalCount > list?.length) {
      dispatch(getExtraServiceListHandle({ skipCount: list?.length }));
    }
  };
