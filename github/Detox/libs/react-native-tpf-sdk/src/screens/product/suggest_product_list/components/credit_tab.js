...
import ProductItem from '../../../credit/components/product_item';
...
const CreditTab = props => {
  ...
  const onFetchProductList = async page => {
    ...
    // const rs = await apiGetLoanProductList({productType: 1, skipCount, maxResultCount: PAGE_SIZE})
    ...
    if (rs?.data?.success) {
      const { items, totalCount } = rs.data.result.products;
  ...
  const renderItem = ({ item, index }) => {
    return <ProductItem item={item} isHot={index < 3} route={route} />;
  };
  const onRefresh = () => {
