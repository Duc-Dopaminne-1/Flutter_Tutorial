...
import InsuranceProduct from '../../../insurance/components/product';
...
const InsuranceTab = props => {
  ...
  const onPress = (event, item) => {
    const { topenId } = store.getState().auth;
    handleTouchItem(event, 'InsuranceItem', route, { ...item, categoryId: item?.id }, topenId);
