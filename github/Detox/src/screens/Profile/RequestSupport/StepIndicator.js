...
const StepIndicator = ({
  ...
  const renderStepIndicator = () => {
    ...
    for (let position = 0; position < stepCount; position++) {
      steps.push(
        <View key={position} style={[styles.stepContainer, HELPERS.row]}>
          {renderStep(position)}
        </View>,
      );
    }
  ...
  const renderStepLabels = () => {
    ...
    const labelViews = labels.map((label, index) => {
      const unfinish = getStepStatus(index) === STEP_STATUS.UNFINISHED;
      const selectedStepLabelStyle =
        index === currentPosition
          ? {color: customStyles.currentStepLabelColor}
          : {color: customStyles.labelColor};
      const labelColor = unfinish ? {color: COLORS.GRAY_A3} : {color: COLORS.BLACK_31};
      return (
        <View key={index} style={styles.stepLabelItem}>
          <Text style={[styles.stepLabel, selectedStepLabelStyle, labelColor]}>{label}</Text>
        </View>
      );
    });

    return (
      <View style={[styles.stepLabelsContainer, {alignItems: customStyles.labelAlign}]}>
        {labelViews}
      </View>
    );
  };
  ...
  const renderStep = (position: number) => {
    ...
    return (
      <Animated.View key={'step-indicator'} style={[styles.step, stepStyle]}>
        {iconName && <Icon size={10} name={iconName} color={COLORS.NEUTRAL_WHITE} />}
      </Animated.View>
    );
  };
  ...
  const getStepStatus = (stepPosition: number) => {
    if (
      labels[stepPosition] === SUPPORT_REQUEST_STATUS.RECEIVED.cancelled ||
      labels[stepPosition] === SUPPORT_REQUEST_STATUS.RECEIVED.reject
    ) {
