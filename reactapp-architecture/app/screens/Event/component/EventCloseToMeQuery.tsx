...
export class EventCloseToMeQuery extends React.PureComponent<Props, State> {
  ...
  fetchEvent = () => {
    const { globalEventFactory, tab } = this.props

    const [subscription, result] = globalEventFactory.fetch({
      tab,
    ...
    this._subscription = subscription.subscribe(globalEvent => {
      if (globalEvent) {
        const data = safeLocation.detectEvent(globalEvent.col)

        this._paging.update(data)

        const newEvents = this._paging.getDataFromStart(
          globalEvent.change.insertions.length === 0
        ...
        if (data.length >= 0 && this._keyword.trim().length === 0) {
          this.setState(
            {
              events: newEvents,
              change: globalEvent.change,
              loading: false,
        ...
        } else if (data.length >= 0 && this._keyword.trim().length !== 0) {
  ...
  onChangeText = debounce((keyword: string) => {
  ...
  render() {
    const { events, denyLocation } = this.state

    if (devices.isAndroid && safeLocation.denyLocation) {
      // location Android
      return (
        <EventCloseToMe
          requestLocation={this.requestLocation}
          icon={images.noInternet}
          textButton={I18n.t('enableLocation')}
          textOne={I18n.t('locationDisabled')}
          textTwo={I18n.t('accessLocation')}
          textThree={I18n.t('detectNearbyTrade')}
        />
      )
    }
    if (denyLocation) {
