import { Supplier, Task } from '@/models/team'
...
export class MultiSearchTaskQuery extends React.PureComponent<Props, State> {
  ...
  componentWillUnmount() {
    ...
    this._fetchTaskSub && this._fetchTaskSub.unsubscribe()
    this._fetchKeywordSub && this._fetchKeywordSub.unsubscribe()
  }

  fetchTask = () => {
    const { taskFactory } = this.props
    const [subscription] = taskFactory.fetch({})

    this._fetchTaskSub = subscription.subscribe(tasks => {
      /**
       * Update new task list to Fuse
       */
      this.searchUpdate(tasks)
  ...
  updateStateOutSide = (state: boolean) => {
    ...
    updateState && updateState('emptyTaskSearchResult', state)
  }

  onPressItem = (task: Task) => {
    const { navigation } = this.props
    navigation.navigate('TasksInfoScreen', { task })
  }
    ...
    return (
      <View style={styles.wrapButton}>
        <TouchableOpacity
          activeOpacity={0.8}
          onPress={() => {
            multiSearchStore.changeTabView(3)
          }}
        >
